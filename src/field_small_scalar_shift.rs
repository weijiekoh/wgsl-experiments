...
async fn field_small_scalar_shift(input_bytes: &[u8]) -> Option<Vec<u32>> {
    ...
    let (_, _, device, queue, compute_pipeline, mut encoder) = device_setup_default("src/field_small_scalar_shift.wgsl").await;
...
pub fn test_field_small_scalar_shift() {
    ...
    // Send to the GPU
    let result = pollster::block_on(field_small_scalar_shift(&input_as_bytes)).unwrap();
