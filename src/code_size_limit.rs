...
async fn code_size_limit(input_bytes: &[u8]) -> Option<Vec<u32>> {
    let num_inputs = input_bytes.len() / 4;

    let (_, _, device, queue, compute_pipeline, mut encoder) = device_setup_default("src/code_size_limit.wgsl").await;
...
pub fn test_code_size_limit() {
    ...
    // Send to the GPU
    let result = pollster::block_on(code_size_limit(&input_as_bytes)).unwrap();
