...
async fn field_reduce(input_bytes: &[u8]) -> Option<Vec<u32>> {
    let num_inputs = input_bytes.len() / 4;
    let (_, _, device, queue, compute_pipeline, mut encoder) = device_setup_default("src/field_reduce.wgsl").await;
...
pub fn test_field_reduce() {
    ...
    // Send to the GPU
    let result = pollster::block_on(field_reduce(&input_as_bytes)).unwrap();
